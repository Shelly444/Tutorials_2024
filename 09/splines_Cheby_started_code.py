...
for n in range(N0,N1+1):
    N = 2**n                                                      # Fix nr. of knots.
    x = np.linspace(-2, 2, N+1)                                   # The locations of the knots.
    y = f(x)                                                       # The y-values at the knots.
    S = intp.CubicSpline(x, y)                                    # Using the spline function from SciPy.interpolate.
    ys = S.__call__(xs)                                           # This SciPy function returns a PPoly object, this is how to evaluate the interpolant.
    plt.plot(xs,f(xs),'-k',label='f(x)')
    plt.plot(xs,ys,'-r',label='cubic spline on '+str(N)+' knots') # Plot with legend.
    plt.ylim([-2,2])
    ...
    plt.legend()
    plt.show()
    e = abs(f(xs)-ys)                                              # Estimate the error on the fine grid.
    plt.title('|f(x)-S_N(x)| for N='+str(N))
    plt.xlabel('x')
    plt.ylabel('error')
    plt.plot(xs,e,'-r')
...
# That still doesn't look nice, the error increases near the boundary. Try different nodes..

for n in range(N0,N1+1):
    N = 2**n
    x = 2.0*np.cos(np.linspace(0,N,N+1) * np.pi/float(N))           # Set the interpolation nodes.
    x.sort()                                                         # Ensure x is strictly increasing
    y = f(x)                                                         # Compute the corresponding y-values.
    ys = intp.CubicSpline(x, y).__call__(xs)                        # Use an in-built function or our own..
    plt.plot(xs,f(xs),'-k',xs,ys,'-r')
    plt.ylim([-2,2])
    plt.show()
    e = abs(f(xs)-ys)
